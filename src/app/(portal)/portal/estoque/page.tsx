...
import { LOW_STOCK, stockTone } from "@/components/InventoryTable";
...
import { createManualProduct, moveStock, REASON_LABEL, REASONS, type Reason } from "@/lib/inventory";
...
async function movimentar(formData: FormData) {
  ...
  if (!REASONS.includes(reason)) redirect("/portal/estoque?erro=Motivo inválido.");
  ...
  redirect(
    result.ok
      ? `/portal/estoque?ok=${encodeURIComponent(`Estoque atualizado para ${result.stock} unidade(s).`)}`
      : `/portal/estoque?erro=${encodeURIComponent(result.message ?? "Não foi possível atualizar.")}`,
  );
}
...
async function novoProduto(formData: FormData) {
  ...
  if (!title || !accountId) redirect("/portal/estoque?erro=Informe o produto e a loja.");
  ...
  try {
    ...
    redirect(`/portal/estoque?erro=${encodeURIComponent(err instanceof Error ? err.message : String(err))}`);
  }

  revalidatePath("/portal/estoque");
  redirect("/portal/estoque?ok=Produto criado.");
}
      ...
      <main className="flex-1 px-6 py-8">
        ...
        <div className="my-5 flex w-fit flex-wrap gap-1.5 rounded-xl border border-line bg-surface p-1">
          {FILTERS.map((f) => (
            <Link
              key={f.key}
              href={f.key === "todos" ? "/portal/estoque" : `/portal/estoque?filtro=${f.key}`}
          ...
          <Card>
            ...
            {accounts.length === 0 ? (
              <p className="px-5 py-5 text-[13px] text-ink-muted">
                ...
                <Link href="/portal/integracoes" className="font-semibold text-brand hover:underline">
                  Integrações
                </Link>
                .
              </p>
